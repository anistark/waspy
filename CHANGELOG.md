...
### Added
- **A method call reaches the override the receiver's runtime class defines.** Dispatch was static everywhere: a call compiled to the implementation the receiver's *declared* class holds, so `self.kind()` inside a base method reached the base's version even when the instance was a subclass that overrode it, and the plain template-method pattern answered `"media:b"` where CPython answers `"video:b"` while reporting success. A method name some class overrides now gets a column in a per-class dispatch table, indexed by the class tag every instance already carries at offset 0, and the call becomes a `call_indirect`: one load and three arithmetic ops. `@property` getters and `__eq__` behind `==` had the same defect through the same route and go through the same table. `super().method()` stays non-virtual, as in Python, so an override can still extend its base
- Only a name some subclass actually overrides is dispatched this way. A method nothing overrides, a class with no subclasses, and a program with no classes at all keep the direct `call` they have always had and emit no table, so nothing that was already correct changes shape
- `examples/oop_polymorphism.py`, a notification hierarchy with a `render()` written once in the base, a list holding three runtime classes at one call site, and a `super()` chain. It is part of `just verify-runtime`, so the dispatch table is exercised under Node and the `wasmtime` CLI at both optimization levels rather than only the in-process harness
...
### Security
- The README documents the `waspy_host` file interface in full (the four imports, their signatures, the mode flag bits) and states plainly that **the embedder owns confinement**: the compiled program picks the path string it passes to `waspy_host.open`, so a host that forwards it to a real filesystem hands the program whatever the process can reach. Nothing in the module constrains that, and the compiler cannot, since the path is a runtime value. The reference hosts in this repo are in-memory test fixtures, not a sandbox to copy
- CI installs `wasmtime` from a pinned release asset with a checked SHA-256, instead of piping `wasmtime.dev/install.sh` to `bash`. The script was fetched fresh on every run, so what the required runtime gate executed was whatever that URL served at the time, and the version moved on its own

### Fixed
- **A value written into a collection is stored at the width the collection is read back at** ([#123](https://github.com/anistark/waspy/issues/123)). Every write path stored whatever the value expression produced, so an int written into a `List[float]` put four bytes in an eight-byte slot: `xs.append(3)` on `[1.0, 2.0]` read back as `1.5e-323`, and `xs[0] = 3`, which overwrote the slot's low half, as `1.0000000000000007`. `append`, `insert`, item assignment, a dict value, and `set.add` all widen now. The type hint alone was not enough, since it is consumed by constants and arithmetic and an int arriving through a variable came out an int whatever the destination asked for, so the conversion happens at the write itself
- An unannotated local takes `int` or `bool` from the value assigned to it, as it already took every pointer-shaped type. Both live in the same i32 slot, so the local layout is unchanged; what it fixes is a value passing through a local on its way into a float collection, which reached the write as untyped and one word wide

### Changed
- **An override that changes the signature is refused.** Every implementation of an overridden name is reached through one `call_indirect`, which names a single WebAssembly type, so the implementations have to agree on their parameters and return type. Python does not require that; here it is reported at compile time, naming both methods, rather than left to trap on the call
- **A write the compiler cannot make good on is refused instead of stored at the wrong width** (#123). A float into a collection whose elements are read as integers would have to truncate (CPython keeps the 2.5; an int slot cannot), a float into a collection with no element type cannot be read back at all, a value of unknown type has no width to widen from, and `list.extend()` between lists read at different widths copies raw slots across. Each names the problem and what to do about it. The mixed-width *literal* check has always taken this stand; the runtime write paths now take it too
