...
- `examples/nested_collections.py`, covering nested list-of-lists indexing, a per-iteration list literal that escapes its loop, and float dict/set values; the integration suite asserts each result
...
- Float values in `dict` are stored width-aware (narrowed to f32 in the one-word slot, like list/tuple elements) and recovered on read (`f32.reinterpret`/`f64.promote`); previously the dict store emitted an `i32.store` of an `f64`, so any float-valued dict produced an invalid module
- Float members of a `set` are reduced to their f32 bit pattern before the i32 dedup search, so `{1.5, 1.5, 2.5}` de-duplicates to two members instead of aborting validation with an `i32`/`f64` type mismatch
