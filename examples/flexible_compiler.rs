...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        ...
        println!(
            "The compiler will generate both optimized and unoptimized versions for comparison"
        );
        return Ok(());
    }
    ...
    let unopt_path =
        python_file.with_file_name(format!("{}.raw.wasm", file_stem.to_string_lossy()));
    ...
    let size_reduction =
        100.0 * (1.0 - (optimized_wasm.len() as f64 / unoptimized_wasm.len() as f64));
