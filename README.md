...
- A lambda's parameters carry no type, so indexing one, calling `len()` on one, or calling a string method on one inside the lambda body reads the value wrongly. `sorted()` refuses a `key=` whose result type it cannot determine rather than miscompiling it, which is why sorting an explicit list of `(-count, word)` tuples is the working shape and `key=lambda kv: (-kv[1], kv[0])` is not
